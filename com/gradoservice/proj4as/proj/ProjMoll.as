    ...
    ----------------------------------------------------*/
		  override public function forward(p:ProjPoint):ProjPoint
		  {
		    ...
		    for (var i:int=0;;i++) {
		       ...
		       theta += delta_theta;
		       if (Math.abs(delta_theta) < ProjConstants.EPSLN) break;
		       if (i >= 50) {
		    ...
		    /* If the latitude is 90 deg, force the x coordinate to be "0 + false easting"
		       ...
		       --------------------------------------------------------------------------*/
		    if (ProjConstants.PI/2 - Math.abs(lat) < ProjConstants.EPSLN) delta_lon =0;
		 ...
		 override public function inverse(p:ProjPoint):ProjPoint
		 {
		    ...
		    if(lon < (-ProjConstants.PI)) lon= -ProjConstants.PI;
		    if(lon > ProjConstants.PI) lon= ProjConstants.PI;
