	...
	public class ProjSoMerc extends AbstractProjProjection
	{
		...
		public function ProjSoMerc(data:ProjParams)
		{
		  ...
		  override public function forward(p:ProjPoint):ProjPoint
		  {
		    ...
		    var rotI:Number = Math.atan(Math.sin(I)
		            / (Math.sin(this.b0) * Math.tan(b) +
		               Math.cos(this.b0) * Math.cos(I)));
		
		    var rotB:Number = Math.asin(Math.cos(this.b0) * Math.sin(b) -
		 ...
		 override public function inverse(p:ProjPoint):ProjPoint
		 {
		    ...
		    var b:Number = Math.asin(Math.cos(this.b0) * Math.sin(rotB)
		            + Math.sin(this.b0) * Math.cos(rotB) * Math.cos(rotI));
		    var I:Number = Math.atan(Math.sin(rotI)
